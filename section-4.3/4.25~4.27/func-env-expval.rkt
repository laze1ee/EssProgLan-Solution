...
(provide function func
         ...
         num-val bool-val func-val subfunc-val
         expval->num expval->bool expval->func expval->subfunc
         expval->any)
...
(define-datatype subfunction subfunction?
  (subfunc
   [vars (list-of identifier?)]
   [body statement?]
   [env list?]))
...
(define expval->subfunc
  (lambda (val)
    (cases expval val
      [subfunc-val (subfunc) subfunc]
      [else (extract-expval-error val 'subfunc-val)])))
